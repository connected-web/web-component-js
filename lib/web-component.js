...
(function () {
  ...
  function stillWaitingForRequirements () {
    var stillWaiting = false
    Object.keys(requiredScripts).forEach((key) => {
      if (!window[key]) {
        stillWaiting = true
      }
    })
  ...
  function runDeferredScript (callback) {
    (function F() {
  ...
  Component = (function () {
    ...
    Class.findByElement = function(element) {
      if(!element) {
        return false;
      }

      var webComponentId = element.getAttribute('webcomponentid');
      var elementName = element.tagName.toLowerCase();

      if(Class.registeredComponents[elementName]) {
        return Class.registeredComponents[elementName].registeredInstances[webComponentId];
      }
    ...
    Class.create = function (elementName, templateElement) {
      ...
      ComponentClass.prototype.render = function (reason) {
        ...
        var expandedTemplate = Component.expandTemplate(this, $(this.template).html(), elementName);
    ...
    Class.expandTemplate = function (data, template, context) {
      if (template) {
        ...
        try {
          ...
          expandedTemplate = handlebarsTemplate(data);
        } catch (ex) {
          var templateDebug = template.replace(/</g, '&lt;').replace(/>/g, '&gt;');
          expandedTemplate = '<h5>Handlebars Template Error:</h5>'
            + '<pre>' + ex + '</pre>'
            + '<h5>Source Template: ' + context + '</h5>'
            + '<pre>' + templateDebug + '</pre>';
        }
      }
